...
mod bmf_tests {
    ...
    fn test_optimum_sphere() {
        ...
        let optimum_fitness = problem.evaluate_solution(&optimum_position).value();
        assert_float_eq!(
    ...
    proptest! {
        ...
        fn test_random_input_sphere(x in prop::collection::vec(-1.0..1.0, 1..30)) {
            ...
            let random_fitness = problem.evaluate_solution(&x).value();
    ...
    proptest! {
        ...
        fn test_random_input_alpine_n2(x in prop::collection::vec(-1.0f64..1.0f64, 1..30)) {
            ...
            let random_fitness = match x.len() {
                1 => (problem.evaluate_solution(&x).value() * 1000.0).trunc() / 1000.0,
                _ => problem.evaluate_solution(&x).value(),
            };
            prop_assert!(random_fitness >= problem.known_optimum());
        }
    }
    ...
    fn test_optimum_bird() {
        ...
        let optimum_fitness1 = problem.evaluate_solution(&optimum_position1).value();
        let optimum_fitness2 = problem.evaluate_solution(&optimum_position2).value();
        assert_float_eq!(
            problem.known_optimum(),
            optimum_fitness1,
            abs <= abs_tol(),
